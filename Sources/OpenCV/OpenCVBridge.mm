...
+ (cv::Mat)resizeToWidth:(cv::Mat)src width:(int)width;
@end
...
// ---------------------------------------------------------------------------
// Private helper: resize preserving aspect ratio
// ---------------------------------------------------------------------------

@implementation OpenCVBridge (ResizeHelper)

+ (cv::Mat)resizeToWidth:(cv::Mat)src width:(int)width {
    if (src.cols == width) return src;
    int height = (int)((float)src.rows * ((float)width / (float)src.cols));
    cv::Mat dst;
...
@interface SIFTFeatures ()
@property (nonatomic, readwrite) NSInteger count;
@property (nonatomic, readwrite) CGSize procSize;
@property (nonatomic, strong, readwrite) NSData *keypointsXY;
@property (nonatomic, strong, readwrite) NSData *descriptors;
@property (nonatomic, strong, readwrite) NSData *scales;
@property (nonatomic, strong, readwrite) NSData *orientations;
@end

@implementation SIFTFeatures
@end

// ---------------------------------------------------------------------------
// OpenCVBridge (SIFTExtraction)
// ---------------------------------------------------------------------------

@implementation OpenCVBridge (SIFTExtraction)

+ (nullable SIFTFeatures *)extractSIFTFrom:(UIImage *)image
                                       topK:(int)topK
                                  procWidth:(int)procWidth {
    cv::Mat bgrMat  = [[self class] matFromUIImage:image];
    cv::Mat procMat = [[self class] resizeToWidth:bgrMat width:procWidth];

    cv::Mat grayMat;
    cv::cvtColor(procMat, grayMat, cv::COLOR_BGR2GRAY);
    ...
    std::vector<cv::KeyPoint> kps;
    cv::Mat descs;  // N × 128 float32, L2-normalised by OpenCV
    sift->detectAndCompute(grayMat, cv::noArray(), kps, descs);

    if (kps.empty() || descs.empty()) return nil;

    int N = (int)kps.size();
    CGSize ps = CGSizeMake(procMat.cols, procMat.rows);

    // RootSIFT: L1-normalise each row then sqrt element-wise.
    for (int i = 0; i < N; ++i) {
        float *row = descs.ptr<float>(i);
        float l1 = 0.0f;
        for (int d = 0; d < 128; ++d) l1 += row[d];
        if (l1 > 1e-8f) {
            for (int d = 0; d < 128; ++d) row[d] = sqrtf(row[d] / l1);
        }
    }

    std::vector<float> kpBuf(N * 2);
    std::vector<float> scalesBuf(N);
    std::vector<float> orientBuf(N);
    static const float kDegToRad = M_PI / 180.0f;
    for (int i = 0; i < N; ++i) {
        kpBuf[i * 2]     = kps[i].pt.x;
        kpBuf[i * 2 + 1] = kps[i].pt.y;
        scalesBuf[i]     = kps[i].size;
        orientBuf[i]     = kps[i].angle * kDegToRad;
    }

    SIFTFeatures *result = [[SIFTFeatures alloc] init];
    result.count        = N;
    result.procSize     = ps;
    result.keypointsXY  = [NSData dataWithBytes:kpBuf.data()
                                         length:kpBuf.size() * sizeof(float)];
    result.descriptors  = [NSData dataWithBytes:descs.data
                                         length:(size_t)N * 128 * sizeof(float)];
    result.scales       = [NSData dataWithBytes:scalesBuf.data()
                                         length:scalesBuf.size() * sizeof(float)];
    result.orientations = [NSData dataWithBytes:orientBuf.data()
                                         length:orientBuf.size() * sizeof(float)];
    return result;
}

@end  // OpenCVBridge (SIFTExtraction)
